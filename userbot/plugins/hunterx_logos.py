...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Hunter X User"
hunter = borg.uid
...
async def _(mafiaevent):
    event = await edit_or_reply(hunterevent, "`Processing.....`")
    fnt = await get_font_file(hunterevent.client, "@D3VIL_FONTSS")
    if hunterevent.reply_to_msg_id:
        rply = await hunterevent.get_reply_message()
    ...
    text = hunterevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    await bot.send_file(
        mafiaevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『[{DEFAULTUSER}](tg://user?id={hunter})』\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
