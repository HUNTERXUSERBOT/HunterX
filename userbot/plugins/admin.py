...
async def promote(promt):
    ...
    hunterevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await hunterevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    hunterevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        return
    await hunterevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    hunterevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await hunterevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await hunterevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await hunterevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    hunterevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await hunterevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await hunterevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    hunterevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await hunterevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        ...
        await hunterevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
